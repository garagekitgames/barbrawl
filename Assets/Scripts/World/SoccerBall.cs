    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        else if (collision.gameObject.layer == LayerMask.NameToLayer("Ground"))
        {
            if (collision.contacts[0].normal.y > 0.6f && collision.relativeVelocity.y > smokeCollisionSpeedCutOff && Time.time - lastSmokeTime > 0.1f)
            {
                ...
                EffectsController.Instance.CreateSmokePuffs(smokeCollisionPuffCount + Random.Range(0, smokeCollisionPuffCount / 2), new Vector3(transform.position.x, collision.contacts[0].point.y, transform.position.z), 3, Vector3.up, smokeCollisionOffset);
            }
        }
    }
}
