		...
		{		
            ...
            if (targets.Length == 1) {
    			...
    			if (dialogue.selectedDialogueID != 0) {    				
					...
					// 3D or 2D Dialogue = World Space or simply on Screen
					dialogue.worldSpace = EditorGUILayout.Toggle("3D Worldspace", dialogue.worldSpace);

					if (!dialogue.worldSpace) {
						// Dialogue Position in 2D Space
						...
						// Dialogue Position in 3D Space
						try { 
							if (GUILayout.Button("Dialogue Position")) { 
								UnityEditor.Selection.activeGameObject = dialogue.GetGameObject(); 
    				...
    				dialogue.animate = EditorGUILayout.Toggle("Animated Popup", dialogue.animate);
                    ...
                    EditorGUILayout.BeginVertical(EditorStyles.helpBox);
                    dialogue.audioClip = (AudioClip)EditorGUILayout.ObjectField(dialogue.audioClip, typeof(AudioClip), true);
                    EditorGUILayout.EndVertical();
                ...
                if (allHaveDialogues) {
                    ...
                    if (EditorGUI.EndChangeCheck()) {
                        for (int i = 1; i < targets.Length; i++) {
                            StepNode currentTarget = (StepNode)targets[i];
                            currentTarget.dialogue.audioClip = dialogue.audioClip;
