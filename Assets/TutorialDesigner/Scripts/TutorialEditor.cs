...
namespace TutorialDesigner {
	...
	public class TutorialEditor : EditorWindow {
        ...
        [MenuItem("Window/Tutorial Designer/Tutorial Editor")]
        static void ShowEditor() {
            GetWindow<TutorialEditor>();
        }

#if !TD_MOD_TMPro
		[MenuItem("Window/Tutorial Designer/Load TextMeshPro Module")]
		static void LoadTDMod() {
			if (Utilities.Various.NamespaceExists("TMPro")) {
				if (EditorUtility.DisplayDialog("Load TextMeshPro Module for TutorialDesigner",
											   "This will add \"TD_MOD_TMPro\" to Scripting Define Symbols. " +
												"Dialogues will use TextMeshPro instead the normal UI Text objects from now on.", "Continue", "Cancel")) {
					Utilities.ModuleLoader.LoadTMPro();
				} 
            } else {
                EditorUtility.DisplayDialog("TextMeshPro missing", "The namespace TMPro cannot be found. Please make sure TextMeshPro is installed within this project, " +
                    "before activating this module.", "OK");
            }
		}
#elif TD_MOD_TMPro
        [MenuItem("Window/Tutorial Designer/Unload TextMeshPro Module")]
        static void UnloadTDMod() {
            if (EditorUtility.DisplayDialog("Unload TextMeshPro Module from TutorialDesigner",
                                                           "This will remove \"TD_MOD_TMPro\" from Scripting Define Symbols. " +
                                                            "Please only execute if you didn't use TMPro in any of your scenes within this project. " +
                                                            "Otherwise it could be broken.", "Unload", "Cancel")) {
                Utilities.ModuleLoader.UnloadTMPro();
            }
        }

        /// <summary>
        /// Updates the Tutorial Designer to newer API
        /// </summary>
        public void UpdateTD() {
            // PreCheck
            GameObject go = GameObject.Find("TutorialSystem");
            SavePoint sp = null;
            if (go != null) {
                sp = go.GetComponent<SavePoint>();
                if (sp != null) {
                    if (sp.updateNeeded != null) {
                        if (sp.updateNeeded.Value) {
                            List<TMP_FontAsset> fonts = new List<TMP_FontAsset>();
                            string[] guids = AssetDatabase.FindAssets("t:TMP_FontAsset", null);
                            foreach (string guid in guids) {
                                TMP_FontAsset newFont = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>(AssetDatabase.GUIDToAssetPath(guid));
                                if (!fonts.Contains(newFont)) fonts.Add(newFont);
                            }

                            if (EditorUtility.DisplayDialog("Tutorial Designer Update",
                                                        "Text Objects will be replaced by TextMeshPro Objects.\n" +
                                                        "Please convert your Fonts to TMP_FontAssets, otherwise it will be changed to Arial.\n" +
                                                        "You can still change Fonts later. Continue?",
                                                        "Yes, I've made a backup",
                                                            "Cancel")) {
                                Dialogue.dialoguePrefabs = Utilities.Various.GetFileList(TDPath + "Resources/Dialogues", "prefab", "No Dialogue", "TMP");

                                if (sp.nodes != null) {
                                    if (sp.nodes.Count > 0) {
                                        foreach (Node node in sp.nodes) {
                                            if (node is StepNode) {
                                                StepNode sn = (StepNode)node;
                                                sn.dialogue.Text2TMP(fonts);
                                            }
                                        }
                                    }
                                }

                                sp.updateNeeded = false;
                            } 
                        }
                    }
                }
        ...
        public void Initialize() {            
            ...
            Dialogue.dialoguePrefabs = Utilities.Various.GetFileList(TDPath + "Resources/Dialogues", "prefab", "No Dialogue", "TMP");
#endif
            Utilities.ModuleLoader.CheckAndFixSymbols("TD_MOD_TMPro");
        ...
        void OnGUI() {
            ...
            if (savePoint != null) {
                ...
                if (tutSysObj != null) {
                    ...
                    if (savePoint.nodes != null) {
#if TD_MOD_TMPro
                        // Snippets to check if there are Text Objects which need to be converted to TMPro
                        ...
                        foreach (Node n in savePoint.nodes) {
                            n.ResetSkin();
#if TD_MOD_TMPro
                            if (savePoint.updateNeeded == null) {
                                // Check if TD Update is necessary
                                if (n is StepNode) {
                                    StepNode sn = (StepNode)n;
                                    if (sn.dialogue.NeedsTMPUpdate()) {
                                        updateNeeded = true;
                                    }
                                }
                            }
...
#if TD_MOD_TMPro
                        savePoint.updateNeeded = updateNeeded;
                        if (updateNeeded) Debug.LogWarningFormat("<b><color=red><size=12>You need to convert Texts to TextMeshPro</size></color></b>\n\r" +
                                                                 "In the TutorialEditor - SideWindow click the 'Convert Texts to TMPro' button");
#endif
                    }
                ...
                if (savePoint != null && !EditorApplication.isPlaying) {
                    ...
                    if (clickedNode == null) {
                        ...
                        if (e.button == 0) {
                            if (!e.shift) {
                                selectedNodes.Clear();
                                UnityEditor.Selection.objects = new Object[] { clickedNode };
                            } else {
                                ...
                                UnityEditor.Selection.objects = selectedNodes.ToArray();
                            }
                        }
        ...
        private void ContextCallback(object obj) {
            ...
            switch (obj.ToString()) {
                ...
                if (dup_node != null) {
#if TD_MOD_TMPro
                        if (dup_node is StepNode) {
                            if (((StepNode)dup_node).dialogue.NeedsTMPUpdate()) {
                                Debug.LogWarningFormat("<b><color=red><size=12>You need to convert Texts to TextMeshPro</size></color></b>\n\r" +
                                                                 "In the TutorialEditor - SideWindow click the 'Convert Texts to TMPro' button");
                                break;
                            }
                        }
                        ...
                        if (dup_node is StepNode) { // Stepnode	
